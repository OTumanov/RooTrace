...
**Интерфейс HttpServer:**
```typescript
export interface HttpServer {
  /**
   * Запускает HTTP сервер
   * @param port Порт для прослушивания (0 для случайного)
   * @returns Промис, который разрешается когда сервер запущен
   */
...
**Интерфейс DashboardManager:**
```typescript
export interface DashboardManager {
  /**
   * Открывает или показывает панель дашборда
   * @param viewColumn Колонка для отображения
   */
  open(viewColumn?: vscode.ViewColumn): Promise<void>;
  
  /**
   * Отправляет лог в дашборд
   * @param log Лог для отображения
   */
  sendLog(log: RuntimeLog): Promise<void>;
  
  /**
   * Отправляет начальные логи при открытии
   * @param logs Массив логов
   */
  sendInitialLogs(logs: RuntimeLog[]): Promise<void>;
  
  /**
   * Закрывает панель дашборда
   */
  close(): void;
...
### 11. `extension/utils/` - Специфичные утилиты
**Файлы:**
- `html-escape.ts` - `escapeHtml()` (специфично для Dashboard HTML)

**Ответственность:**
- Специфичные для extension утилиты
...
- ⚠️ **НЕ дублировать общие утилиты!** Использовать из `src/utils/`:
  ...
  - `parseArrayOrDecrypt()` → `utils/config-parser.ts`
...
1. Создать структуру директорий `src/extension/`
2. Определить интерфейсы между модулями (см. детализацию выше)
3. Написать план миграции (см. `00-migration-strategy.md`)

### Этап 2: Извлечение специфичных утилит (0.5 дня)
1. Вынести `html-escape.ts` (специфично для Dashboard)
2. Протестировать
...
- [ ] `http-server.test.ts` - тесты HTTP сервера
- [ ] `server-handlers.test.ts` - тесты обработчиков запросов
- [ ] `rate-limiter.test.ts` - тесты rate limiting
- [ ] `dashboard-manager.test.ts` - тесты Dashboard
- [ ] `dashboard-message-handler.test.ts` - тесты обработки сообщений
- [ ] `command-registry.test.ts` - тесты регистрации команд
- [ ] `log-manager.test.ts` - тесты логирования
- [ ] `config-manager.test.ts` - тесты конфигурации

#### Интеграционные тесты:
- [ ] `extension.integration.test.ts` - полный цикл расширения
  - Активация → запуск сервера → получение логов → отображение в Dashboard
  - Команды VS Code
  - WebSocket обновления

#### E2E тесты:
- [ ] `e2e-extension-workflow.test.ts` - полный workflow расширения
  - Активация → HTTP запрос → лог → Dashboard → команды
...
- **Минимальный MVP**: 15-18 дней (сервер + dashboard + команды + базовое тестирование)
