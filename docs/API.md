...
9. **`mcp--roo-trace--get_problems`** - Получение диагностики (ошибки и предупреждения) из VS Code Problems panel
10. **`mcp--roo-trace--load_rule`** - Загрузка конкретного правила из `.roo/roo-trace-rules/` (и других разрешённых директорий `.roo/rules*`) для lazy loading
