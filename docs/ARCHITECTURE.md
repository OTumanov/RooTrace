...
7. RoleManager использует модульную систему: базовый промпт генерируется в коде, все модули загружаются лениво из `.roo/roo-trace-rules/` через `load_rule`
8. RoleManager генерирует базовый промпт в коде, все модули загружаются лениво из `.roo/roo-trace-rules/` через `load_rule`
...
## Оптимизация контекста AI Debugger

### Гибридная ленивая загрузка (Lazy Loading)

Для оптимизации производительности AI Debugger использует гибридную архитектуру ленивой загрузки:

- **Встроенные модули** (~11 KB):
  - `00-base-language.md` (839 B) - базовые языковые правила
  - `00-base-error-handling.md` (1.11 KB) - обработка ошибок
  - `00-base-penalties.md` (6.01 KB) - система штрафов
  - `00-formats-validator.md` (2.95 KB) - валидация форматов
- **Загружаемые по требованию** (~122 KB) - остальные 36 модулей
- **Общий объем до оптимизации**: 133 KB
- **Общий объем после оптимизации**: ~133 KB (всего) → ~11 KB (встроенные) + ~122 KB (ленивая загрузка)
- **Снижение объема контекста**: ~92%
- **Механизм загрузки**: остальные модули загружаются через `load_rule` по требованию

### Архитектура

- **Ленивая загрузка** позволяет динамически подгружать правила только при необходимости
- **Встроенные базовые модули** обеспечивают минимально необходимый функционал сразу
- **Механизм резервного копирования** создает `.roomodes.backup` для восстановления
