...
2. **Тест 1: Загрузка промпта**
   - Проверьте, что RoleManager загружает инструкции из `prompts/ai-debugger-prompt.en.md` (приоритет) или `prompts/ai-debugger-prompt.md` (fallback)
   - Убедитесь, что инструкции содержат упоминание о кнопках
   - Проверьте, что watcher отслеживает изменения в обоих файлах
   - Убедитесь, что инструкции содержат Phase 2.2: Smoke Test
...
**Загрузка промптов:**
- Система сначала пытается загрузить `prompts/ai-debugger-prompt.en.md` (английская версия)
- Если английской версии нет, используется `prompts/ai-debugger-prompt.md` (русская версия, fallback)
- Watcher отслеживает изменения в обоих файлах и автоматически обновляет `.roomodes`

**Обновление промптов:**
- Изменения в `ai-debugger-prompt.en.md` или `ai-debugger-prompt.md` автоматически подхватываются watcher'ом
- `.roomodes` обновляется автоматически без перезапуска расширения
- Это позволяет править логику экспертной панели "на лету"
