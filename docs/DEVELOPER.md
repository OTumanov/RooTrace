...
2. **Тест 1: Загрузка промпта**
   - Проверьте, что RoleManager использует модульную систему: базовый промпт в коде, модули из `.roo/roo-trace-rules/` загружаются лениво через `load_rule`
   - Убедитесь, что базовый промпт содержит упоминание о кнопках
   - Проверьте, что модули из `.roo/roo-trace-rules/` загружаются через `load_rule`
   - Убедитесь, что модуль `roo-07-smoke-test.md` содержит Phase 2.2: Smoke Test

3. **Тест 1.5: Динамическое обнаружение сети (Phase 2)**
   - Проверьте, что система читает `.debug_port` или `.rootrace/debug_port`
   - Проверьте, что система определяет Docker окружение через `docker ps`
   - Проверьте, что система выполняет smoke test перед Phase 3
   - Убедитесь, что при провале smoke test система не продолжает к инструментации

4. **Тест 2: Интерфейс панели отладки**
   - Запустите команду `ai-debugger.openDashboard`
   - Проверьте наличие кнопок: Run & Reproduce, Analyze Logs, Confirm Fix, Clear Logs

5. **Тест 3: Работа кнопок**
...
- `role-manager.ts` — управление ролями и загрузка промптов (приоритет английской версии)

### Работа с промптами

**Загрузка промптов:**
- Система использует модульную архитектуру: базовый промпт генерируется в `src/role-manager.ts`, все модули находятся в `.roo/roo-trace-rules/` и загружаются лениво через `load_rule` по требованию
- Watcher отслеживает изменения в обоих файлах и автоматически обновляет `.roomodes`

**Обновление промптов:**
- Изменения в модулях `.roo/roo-trace-rules/*.md` доступны сразу после копирования в рабочую область
- Базовый промпт обновляется при изменении `src/role-manager.ts`
- Модули загружаются лениво через `load_rule`, что позволяет обновлять их без перезапуска расширения

**Мета-когнитивные техники:**
- **Аналитик**: Проверяет scope, импорты, экономию токенов, пустоту массивов
- **Критик**: Ищет места, где код упадет (импорты, scope, синтаксис)
- **Изобретатель**: Оптимизирует для лаконичности (5-Line Rule)
...
9. **Используйте английские промпты** - они экономят токены и улучшают следование инструкциям
10. **Следуйте правилам гигиены кода** - The 5-Line Rule, метрики количества, оптимизация импортов
11. **Проводите мета-когнитивную проверку** перед каждым изменением кода
12. **Контролируйте галлюцинации перевода** - используйте правильную терминологию на языке пользователя