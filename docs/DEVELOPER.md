...
3. Обновите документацию в `docs/ARCHITECTURE.md` и `docs/API.md`
...
- `rootrace-dir-utils.ts` — управление директорией `.rootrace/`
...
6. **Всегда используйте `.rootrace/` директорию** для хранения конфигурации и логов
7. **Всегда проверяйте наличие резервной копии** перед редактированием файлов
8. **Запрещайте `inject_probes` для Python** - используйте `apply_diff` (Block Rewrite)