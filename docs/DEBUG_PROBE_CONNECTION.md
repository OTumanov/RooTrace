...
2. **Неправильный URL в коде проб**
   - Решение: Проверьте, что `.rootrace/debug_port` содержит правильный порт (файл в открытом виде). Конфиг в `.rootrace/ai_debug_config` зашифрован для безопасности
   - По умолчанию используется `http://localhost:51234/`, но может быть изменен через `rooTrace.serverPort`
   - Проверьте логи RooTrace (Output > AI Debugger) для подтверждения запуска сервера
