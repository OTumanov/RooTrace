...
1. **VSCode расширение «Debug Sidecar»**
   – запускает локальный MCP-сервер RooTrace на случайном порту
   – принимает POST-запросы с отладочными данными
   ...
   – автоматически создает `.ai_debug_config` с URL сервера

2. **MCP-интеграция (Model Context Protocol)**
   – автоматическая регистрация сервера RooTrace в системе MCP
   – предоставление инструментов для взаимодействия с отладочной системой:
...
3. **WebView Dashboard**
   ...
   – цветовое кодирование по гипотезам (H1 – красный, H2 – зелёный, …)
   – фильтрация и поиск по гипотезам
   – кнопка очистки логов

4. **Roo Code с System Prompt**
   – специальный системный промпт (`prompts/roo-code-system-prompt.md`) описывает протокол Hypothesis‑Driven Debugging
   – Roo Code читает файл `.ai_debug_config` для определения адреса сервера
   – при инструментировании кода Roo Code добавляет блоки `// AI_DEBUG_START … // AI_DEBUG_END` с `fetch`‑запросами на локальный сервер
   – после выполнения сценария Roo Code анализирует логи из канала «AI Debugger» и делает выводы
...
- **`.debug_port`** – простой текстовый файл с номером порта, который слушает сервер (например, `51234`). Создаётся автоматически при запуске сервера, удаляется при остановке.
...
Система переработана для интеграции с Model Context Protocol (MCP). Вместо традиционного HTTP-сервера теперь используется MCP-сервер RooTrace, который обеспечивает более глубокую интеграцию с AI-ассистентами и упрощает взаимодействие через стандартизированный протокол.
...
1. При активации расширения Debug Sidecar происходит автоматическая регистрация MCP-сервера
2. Сервер регистрируется в системе MCP и становится доступным для Roo Code
3. Все инструменты становятся доступны без дополнительной настройки
4. Роутинг вызовов инструментов происходит через MCP протокол
...
2. Удаляет эти блоки, оставляя остальной код неизменным
3. Удаляет файл `.ai_debug_config`, если он существует
4. Показывает уведомление о количестве изменённых файлов и удалённых маркеров

**Внимание:** очистка затрагивает только те файлы, которые были открыты в текущей рабочей области VSCode. Убедитесь, что все нужные файлы находятся в корневой папке проекта.
...
|---------|---------------|----------|
| Start Debug Sidecar Server | `debugSidecar.startServer` | Запускает локальный HTTP‑сервер для приёма отладочных данных. Если сервер уже запущен, показывает соответствующее сообщение. |
| Stop Debug Sidecar Server | `debugSidecar.stopServer` | Останавливает сервер и удаляет файл `.debug_port`. |
| Clear AI Debugger Logs | `debugSidecar.clearLogs` | Очищает канал вывода «AI Debugger» и внутреннее хранилище логов. |
| Open AI Debugger Dashboard | `ai-debugger.openDashboard` | Открывает WebView‑панель с визуализацией логов в реальном времени. |
| Cleanup AI Debug Code | `ai-debugger.cleanup` | Удаляет все блоки кода, помеченные `// AI_DEBUG_START … // AI_DEBUG_END`, из файлов проекта. Также удаляет файл `.ai_debug_config`. |
